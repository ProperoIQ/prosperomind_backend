...
class DataManagementTableRepository(BaseRepository):
    ...
    def get_data_management_table(self, table_id: int) -> Any:
        ...
        """)
        values = {"board_id": table_id}
        table_data_list = self.execute_query_all(query, values)  # Returns a list of tuples
    
        if not table_data_list:
            return []
    
    # Convert tuples to dictionary-based objects
        column_names = list(DataManagementTable.__annotations__.keys())  # Get column names
        table_dicts = [dict(zip(column_names, row)) for row in table_data_list]
    ...
    def get_csv_data_by_board_id(self, board_id: int):
        query = text("""
        SELECT file_download_link, table_name FROM data_dictionay_files
    ...
    """)
        ...
        if not download_links_by_board_id_list:
            return JSONResponse(content={"message": "No data found"}, status_code=404)

        result_dict = {}

        for file_download_link, table_name in download_links_by_board_id_list:
            if table_name not in result_dict:
                result_dict[table_name] = []

        # Read CSV from the link
            df = pd.read_csv(file_download_link)
            result_dict[table_name].append(df.to_dict(orient="records"))  # Convert to JSON
...
class TableStatusRepository(BaseRepository):
    ...
    def update_table_status(self, table_status: TableStatus, upload_df) -> TableStatus:
        # Upload DataFrame to Google Bucket
        bucket_name = 'datamanagementtable'
        current_month_date = datetime.now().strftime("%Y-%m")
        table_status.file_download_link = f'gs://{bucket_name}/{current_month_date}/{table_status.filename}'
        ...
        query = text("""
            ...
            WHERE data_management_table_id = :data_management_table_id
